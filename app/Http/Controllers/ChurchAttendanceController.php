...
class ChurchAttendanceController extends Controller
{
    ...
    public function __construct(ChurchAttendance $attendance, Registration $registration)
    {
    ...
    public function saveAttendance(Request $request)
    {
        ...
        $check = $this->_attendance->_checkAttendance($idNumber, $date);
        if ($check) {
            $data = [
                'success' => true,
                'description' => 'Student/Staff attendance data is already in the system'
            ];
            return response()->json([
                'data' => $data
            ], 201);
    ...
    public function getStudentStaffAttendance(Request $request)
    {
